...
foreach($result as $row)
{
 ...
 <div class="panel panel-default" style="border-radius: 20px;max-width: 650px;min-height:100%;box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19);" align="right">
  ...
  "><b><font color="'.$user_color.'">'.$row["comment_sender_name"].'</font></b></a> on <i>'.$date.'</i> <b><u>('.$row['comment_id'].'</b></u><br>';
  ...
  <div class="panel-footer" align="right" style="border-radius: 0px 0px 20px 20px;">
    ...
    <button type="button" class="btn btn-default reply" id="'.$row["comment_id"].'">Reply to '.$row["comment_id"].'</button>
    <button '.$delete_status.' type="button" class="btn btn-default delete" id="'.$row.["comment_id"].'">Delete </button>
    <input hidden name="ecomment_id" value="'.$row["comment_id"].'">
    <input hidden name="econtent" value="'.$row["comment"].'">
    <button '.$delete_status.' type="submit" class="btn btn-default edit" id="'.$row["comment_id"].'" name="edit-submit">Edit </button>
    </form>
  </div>
 </div>
 ';
